...
def main(
    ...
    if artifacts_dir:
        assert dry_run
        environment_yaml, installer = get_installer_artifacts_local(artifacts_dir)
    else:
    ...
    release_notes = make_release_notes(this_version, environment_yaml)
...
def get_installer_artifacts_local(artifacts_dir):
    """Download the artifacts from a GitHub Actions run
    ...
    :returns: A tuple of the:
        ...
        * The `bytes` corresponding to the DIRACOS2 installer itself
    """
    environment_yaml = (Path(artifacts_dir) / "environment-yaml" / "environment.yaml").read_text()
    installer = next((Path(artifacts_dir) / "installer").glob("DIRACOS*.sh")).read_bytes()
    return environment_yaml, installer
...
def make_release_notes(version, environment_yaml):
